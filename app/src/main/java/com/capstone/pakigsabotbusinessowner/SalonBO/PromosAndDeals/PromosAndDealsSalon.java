package com.capstone.pakigsabotbusinessowner.SalonBO.PromosAndDeals;
...
import com.capstone.pakigsabotbusinessowner.SalonBO.AddPromosAndDealsSalon;
...
public class PromosAndDealsSalon extends AppCompatActivity {
    ...
    SalonPromosAndDealsAdapter salonPADAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_promos_and_deals_salon);
        ...
        userId = user.getUid();
        salonPADArrayList = new ArrayList<>();

        //initializing the arraylist where all data is stored
        salonPADArrayList = new ArrayList<SalonPromosAndDealsModel>();

        //initializing the adapter
        salonPADAdapter = new SalonPromosAndDealsAdapter(PromosAndDealsSalon.this, salonPADArrayList);
        promoAndDealsRecyclerView.setAdapter(salonPADAdapter);

        //to get the data from Firebase Firestore
        getSalonPromosAndDeals();
    ...
    private void refs() {
        addPADBtn = findViewById(R.id.addSalonPADBtn);
        backPADBtn = findViewById(R.id.backBtnSalonPAD);
    }
    ...
    private void addPromoAndDeals(){
        Intent intent = new Intent(getApplicationContext(), AddPromosAndDealsSalon.class);
        startActivity(intent);
    }

    private void getSalonPromosAndDeals() {
        ...
        fStore.collection("establishments").document(userId).collection("salon-promo-and-deals")
                .get()
                .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    @Override
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        salonPADArrayList.clear();

                        for(DocumentSnapshot doc : task.getResult()){
                            SalonPromosAndDealsModel salonPADList = new SalonPromosAndDealsModel(doc.getString("salonPADId"),
                                    doc.getString("salonPADName"),
                                    doc.getString("salonPADDesc"),
                                    doc.getString("salonPADStartDate"),
                                    doc.getString("salonPADEndDate"));
                            salonPADArrayList.add(salonPADList);
                        }
                        salonPADAdapter = new SalonPromosAndDealsAdapter(PromosAndDealsSalon.this, salonPADArrayList);
                        // setting adapter to our recycler view.
                        promoAndDealsRecyclerView.setAdapter(salonPADAdapter);
                        salonPADAdapter.notifyDataSetChanged();
                        progressDialog.dismiss();
                    }
                }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(PromosAndDealsSalon.this, e.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
