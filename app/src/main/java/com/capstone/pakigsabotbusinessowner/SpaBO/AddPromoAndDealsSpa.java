package com.capstone.pakigsabotbusinessowner.SpaBO;
...
import com.capstone.pakigsabotbusinessowner.SpaBO.PromoAndDeals.PromoAndDealsSpa;
...
public class AddPromoAndDealsSpa extends AppCompatActivity {
    ...
    DatePickerDialog.OnDateSetListener spaPADStartDateSetListener, spaPADEndDateSetListener;
    StorageTask uploadTask;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_add_promo_and_deals_spa);
        //References
        refs();
        ...
        padStartDate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                DatePickerDialog dialog1 = new DatePickerDialog(
                        AddPromoAndDealsSpa.this,
                        android.R.style.Theme_Holo_Light_Dialog_MinWidth,
                        spaPADStartDateSetListener,
                        year,month,day);
        ...
        spaPADStartDateSetListener = new DatePickerDialog.OnDateSetListener() {
            @Override
        ...
        spaPADEndDateSetListener = new DatePickerDialog.OnDateSetListener() {
            @Override
            public void onDateSet(DatePicker datePicker, int year, int month, int day ){
                month = month + 1;
                Log.d(TAG, "onDateSet: mm/dd/yy: "  + month + "/" + day + "/" + year);
    ...
    public void refs() {
        padName = findViewById(R.id.spaPADNameTxt);
        padDesc = findViewById(R.id.spaPADDescTxt);
        padStartDate = findViewById(R.id.spaPADESDTxt);
        padEndDate = findViewById(R.id.spaPADEEDTxt);

        backBtn = findViewById(R.id.backBtnAddPromoDeals);
        saveBtn = findViewById(R.id.saveSpaPADBtn);
        padNameLayout = findViewById(R.id.spaPADNameLayout);
        padDescLayout = findViewById(R.id.spaPADDescLayout);
        padEffectiveStartDateLayout = findViewById(R.id.spaPADESDLayout);
        padEffectiveEndDateLayout = findViewById(R.id.spaPADEEDLayout);

    }

    private void spaPromoAndDeals() {
        Intent intent = new Intent(getApplicationContext(), PromoAndDealsSpa.class);
    ...
    private void savePADDetailsToFirestore() {
        ...
        if (txtPADName.isEmpty() || txtPADDesc.isEmpty() || txtPADStartDate.isEmpty() || txtPADEndDate.isEmpty()) {
            Toast.makeText(AddPromoAndDealsSpa.this, "Some fields are EMPTY.", Toast.LENGTH_SHORT).show();
        } else {
            ...
            if (txtPADStartDate.isEmpty()) {
                ...
                Toast.makeText(AddPromoAndDealsSpa.this, "Upload Successful", Toast.LENGTH_SHORT).show();

                //Store promo and deals details
                Map<String, Object> spaPAD = new HashMap<>();
                spaPAD.put("spaPADId", autoId);
                spaPAD.put("spaPADName", txtPADName);
                spaPAD.put("spaPADDesc", txtPADDesc);
                spaPAD.put("spaPADStartDate", txtPADStartDate);
                spaPAD.put("spaPADEndDate", txtPADEndDate);

                //To save inside the document of the userID, under the dental-procedures collection
                fStoreRef.collection("establishments").document(userId).collection("spa-promo-and-deals").document(autoId).set(spaPAD);
