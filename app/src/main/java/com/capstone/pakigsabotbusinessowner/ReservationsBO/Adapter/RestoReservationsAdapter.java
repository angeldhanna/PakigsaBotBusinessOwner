package com.capstone.pakigsabotbusinessowner.ReservationsBO.Adapter;
...
import com.capstone.pakigsabotbusinessowner.ReservationsBO.EmailToCustomer;
import com.capstone.pakigsabotbusinessowner.ReservationsBO.Model.ReservationsModel;
...
public class RestoReservationsAdapter extends RecyclerView.Adapter<RestoReservationsAdapter.MyViewHolder> {
    ...
    int numCompletedReserve;
    String numC;
    int numCompleted;

    public RestoReservationsAdapter() {
        //empty constructor needed
    }

    public RestoReservationsAdapter(Context context, ArrayList<ReservationsModel> reservationsArrayList) {
    ...
    @NonNull
    @Override
    public RestoReservationsAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        //passing layout file to display the items
        View itemView = LayoutInflater.from(context).inflate(R.layout.reservations_items_bo,parent, false);
        return  new MyViewHolder(itemView);
    }

    @SuppressLint("RecyclerView")
    @Override
    public void onBindViewHolder(@NonNull RestoReservationsAdapter.MyViewHolder holder, int position) {//creating  object of ReservationsModel class and setting data to the textviews from the ReservationsModel class
        ...
        holder.details.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                final DialogPlus dialogPlus = DialogPlus.newDialog(holder.reservationName.getContext())
                        .setContentHolder(new ViewHolder(R.layout.resto_reservation_details_popup))
                ...
                TextView preOrderET = viewDP.findViewById(R.id.preOrderET);
                TextView preOrderAmt = viewDP.findViewById(R.id.totalAmtTV);
                ...
                preOrderET.setText(preOrderET.getText().toString() + reservationList.getPreOrderItems());
                preOrderAmt.setText(preOrderAmt.getText().toString() + reservationList.getTotalAmt());
...
*/
                        String message = "This message serves as a verification that your RESERVATION for " + reservationList.getReservePax() +
                                " in " + reservationList.getEstName() + " on " + reservationList.getReserveDateIn() + " at " + reservationList.getReserveTime()
                        + " has been CONFIRMED. Thank you for trusting Pakigsa-Bot!";
        ...
        holder.markAsComplete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //Move the reservation to completed
               fStore.collection("reservations").document(reservationList.getReserveAutoId())
                        .delete()
                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                            @Override
                ...
                docRef = fStore.collection("establishments").document(reservationList.getEstID());
                docRef.addSnapshotListener(new EventListener<DocumentSnapshot>() {
                    @Override
                    public void onEvent(@Nullable DocumentSnapshot value, @Nullable FirebaseFirestoreException error) {
                        numCompletedReserve = value.getLong("est_completedReservations").intValue();
                    }
                });

                //Increment +1 the current number of reservations
                numCompletedReserve = numCompletedReserve + 1;

                DocumentReference docuRef = fStore.collection("establishments").document(reservationList.getEstID());
                Map<String, Object> editedNumReservationsEst = new HashMap<>();
                editedNumReservationsEst.put("est_completedReservations", numCompletedReserve);
                docuRef.update(editedNumReservationsEst).addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(context.getApplicationContext(), "No changes has been made", Toast.LENGTH_SHORT).show();
                    }
                });
